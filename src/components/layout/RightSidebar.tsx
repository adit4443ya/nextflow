  ...
  return (
    <div>
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <ChevronDown
              size={12}
              className={`text-[#52525b] transition-transform ${expanded ? "rotate-180" : ""}`}
            />
          </div>
        </div>
      ...
      {expanded && run.nodeRuns && run.nodeRuns.length > 0 && (
        <div className="ml-2 mt-1 space-y-0.5 border-l-2 border-[#2a2a2e] pl-3 py-1">
          {run.nodeRuns.map((nr) => (
            <div key={nr.id || nr.nodeId} className="text-xs py-1">
              <div className="flex items-center gap-1.5">
                {STATUS_ICON[nr.status]}
                <span className="text-[#a1a1aa] font-medium">{nr.nodeName}</span>
                {nr.duration != null && (
              ...
              {nr.output && (
                <div className="text-[10px] text-[#52525b] mt-0.5 pl-5 truncate max-w-[220px]">
                  Output: {nr.output.length > 80 ? nr.output.slice(0, 80) + "..." : nr.output}
                </div>
              )}
              {nr.error && (
                <div className="text-[10px] text-red-400 mt-0.5 pl-5">
                  Error: {nr.error}
                </div>
              )}
            </div>
...
export default function RightSidebar() {
  ...
  if (collapsed) {
    return (
      <div className="w-12 bg-[#141416] border-l border-[#2a2a2e] flex flex-col items-center py-3">
        <button
        ...
        {runs.length > 0 && (
          <div className="mt-2 w-5 h-5 rounded-full bg-purple-500/20 flex items-center justify-center">
            <span className="text-[9px] text-purple-400">{runs.length}</span>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="w-72 bg-[#141416] border-l border-[#2a2a2e] flex flex-col shrink-0">
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#2a2a2e]">
        <div className="flex items-center gap-2">
          <History size={14} className="text-[#71717a]" />
          <span className="text-sm font-semibold text-[#e4e4e7]">History</span>
          {runs.length > 0 && (
            <span className="text-[10px] text-[#52525b] bg-[#1a1a1e] px-1.5 py-0.5 rounded">
              {runs.length}
            </span>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2">
        {isLoading ? (
          <div className="flex items-center justify-center py-12">
            <Loader2 size={20} className="text-[#52525b] animate-spin" />
          </div>
        ) : runs.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 text-[#52525b]">
            <Clock size={32} className="mb-2 opacity-50" />
            <p className="text-xs">No workflow runs yet</p>
